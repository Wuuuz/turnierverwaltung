...
class SpieleVerteilenController extends Controller
{

    /**
     * @Route("/turnierplan/erstellung/4", name="spVerteilenList")
     */
    ...
    /**
     * @Route("/turnierplan/erstellung/4/edit", name="spVerteilen")
     */

    public function spieleVerteilen()
    {
        ...
        $spielplan
            ...
            ->setSpielzeit($request->request->get('input_spielzeit')+$request->request->get('input_wechselzeit'))
            ->setSpielfeldBelegung($request->request->get('input_spielfelderbelegung'));

        $em = $this->getDoctrine()->getManager();
        $em->persist($spielplan);
        $em->flush();

        $anzSp = $spielplan->getAnzSpFeld();
        $typ = $spielplan->getSpielfeldBelegung();
        $spielzeit = $spielplan->getSpielzeit();
        ...
        for($i = 0; $i < count($altersklassen); $i++)
        {
            $mannschaften = $this->getDoctrine()
                ...
                ->findBy(array('altersklasse' => $altersklassen[$i]));
        ...
        if($typ == 1) {
            ...
            $this->verteileSpieleAufFelder($verteilungsArray,0,$anzSp,$spielzeit,$anfangsZeit, $turniertage);
        ...
        if($typ == 2) //Gleichmäßige Verteilung mit Spielfeldtreue
        {
            for($i = 0; $i < count($verteilungsArray); $i++) {
                $this->verteileSpieleAufFelder($verteilungsArray[$i], $i, 1, $spielzeit,$anfangsZeit,$turniertage);
        ...
        return $this->redirectToRoute('spVerteilenList');
    }
    ...
    public function erzeugeVerteilungsArray($spieltagWrappers, $anzFelder, $typ)
    {
        ...
        usort($spieltagWrappers, array($this, "cmpVerteilerWrapperSpiele"));
     ...
     * @return int
     */
    private function cmpVerteilerWrapperSpiele($vt1, $vt2)
    {
//        if ($vt1->getGeschlec > $vt2->getGeschlecht())
//            return 1;
//        return 0;

        if ($vt1->getAnzSpiele() < $vt2->getAnzSpiele())
            return 1;
        return 0;
    }
}